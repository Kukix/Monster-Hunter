    ...
    class DataHelper
    {
        ...
        public List<Skill> GetSkills(int idCharacter)
        {
            ...
            try
            {
                ...
                string sql = "SELECT * FROM mdw.skill INNER JOIN mdw.character_skill on skill.idPower = character_skill.skill_idPower WHERE character_skill.character_idCharacter =" + idCharacter;
                MySqlCommand command = new MySqlCommand(sql, connection);
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        if (reader.HasRows)
                        {
                            int id = (int)reader.GetInt32("idPower");
                            string name = (string)reader.GetString("PowerName");
                            int strength = (int)reader.GetInt32("Attack");
                            string e = (string)reader.GetString("Element");
                            Element element = (Element)Enum.Parse(typeof(Element), e, true);
                            string description = (string)reader.GetString("Effect Description");
                            int lv = (int)reader.GetInt32("skillLv");
                            //add to the list
                            skills.Add(new Skill(id, name, strength, element, description, lv));
                        }
                    }
                    reader.Close();
                    connection.Close();
